...
export function SidebarInsetWrapper({ children }: { children: React.ReactNode }) {
  return (
    <SidebarInset className="flex flex-col min-w-0 flex-1 h-svh overflow-hidden" style={{
      ...
      <PrestataireHeader />
      <main className="flex flex-col min-h-0 flex-1 w-full px-4 sm:px-5 md:px-6 lg:px-8 pt-5 md:pt-6 pb-28 md:pb-24 lg:pb-32 overflow-x-hidden overflow-y-auto touch-pan-y overscroll-y-contain">
        {children}
      </main>
      <ChatbotAdvisor />
