...
import { CreateEventDialog, EditEventDialog, DeleteConfirmDialog, DayEventsDialog } from './EventDialogs'
...
export function CalendarDashboard({
  ...
  return (
    <div className="flex h-full w-full">
      ...
      <div className="flex-1 flex flex-col h-full">
        ...
        <div className="flex-shrink-0 flex flex-col gap-2 sm:gap-0 px-3 sm:px-5 md:px-6 py-3 sm:py-3.5 bg-white border-b border-gray-200">
          ...
          <div className="flex items-center justify-between sm:hidden">
            ...
            <button onClick={goToToday} className="flex-1 min-w-0 group">
              <h2 className="text-sm font-semibold text-[#823F91] text-center px-1 truncate group-active:opacity-60 transition-opacity">
                {getPeriodTitle()}
              </h2>
            </button>
            <button
            ...
            {(['month', 'week', 'day', 'agenda'] as const).map((viewType) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    'flex-1 py-2 rounded-full text-xs font-medium transition-all active:scale-95 touch-manipulation',
                    viewMode === viewType
      ...
      {/* FAB mobile — Bouton flottant création d'événement */}
      <button
        onClick={() => {
          const rawDate = viewMode === 'day' ? (selectedDate || currentDate) : new Date()
          const dateToUse = normalizeDate(rawDate)
        ...
        className="fixed bottom-6 right-6 z-50 sm:hidden w-14 h-14 flex items-center justify-center bg-[#823F91] text-white rounded-full shadow-lg active:scale-95 transition-transform touch-manipulation"
      >
