 ...
 * Génère le lien de recovery via l'API admin Supabase
 */
export async function sendPasswordResetEmail(email: string) {
  if (!resendApiKey) {
    logger.warn('RESEND_API_KEY non configurée - email de reset non envoyé')
  ...
  try {
    ...
    if (linkError || !linkData?.properties?.action_link) {
      ...
      logger.error('Erreur génération lien de reset:', linkError)
    ...
    const subject = `Réinitialisez votre mot de passe - Nuply`
    ...
    const html = `
      ...
      <head>
        ...
        <title>Réinitialisez votre mot de passe - Nuply</title>
      </head>
                ...
                <tr>
                  <td style="padding: 50px 40px;">
                    ...
                    <p style="margin: 0 0 32px 0; color: #4a5568; font-size: 16px; line-height: 1.6;">
                      Nous avons reçu une demande de réinitialisation de votre mot de passe sur <strong style="color: #823F91;">Nuply</strong>. Cliquez sur le bouton ci-dessous pour choisir un nouveau mot de passe.
                    </p>
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" style="margin: 40px 0;">
                      <tr>
                        <td align="center">
                          <a href="${resetUrl}" style="display: inline-block; padding: 18px 48px; background: linear-gradient(135deg, #823F91 0%, #c081e3 100%); color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 17px; box-shadow: 0 4px 14px rgba(130, 63, 145, 0.4);">
                    ...
                    <p style="margin: 0 0 32px 0; color: #718096; font-size: 14px; line-height: 1.6; text-align: center;">
                      ...
                      <a href="${resetUrl}" style="color: #823F91; word-break: break-all;">${resetUrl}</a>
                    </p>
                    ...
                    <div style="margin: 40px 0 0 0; padding: 24px; background-color: #f3f4ff; border-left: 4px solid #823F91; border-radius: 6px;">
                      <p style="margin: 0; color: #823F91; font-size: 14px; line-height: 1.6;">
                        <strong>⚠️ Important :</strong> Ce lien expire dans <strong>1 heure</strong> pour des raisons de sécurité. Si vous n'avez pas fait cette demande, ignorez cet email.
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="padding: 40px; text-align: center;">
                    <p style="margin: 0 0 20px 0; color: #718096; font-size: 14px; line-height: 1.6;">
                      Vous n'avez pas demandé de réinitialisation ?<br>Vous pouvez ignorer cet e-mail en toute sécurité.
                    </p>
    ...
    logger.info('✅ Email de reset mot de passe envoyé avec succès', { email })
    return { success: true }
  } catch (error: any) {
